...
_DEFAULT_PERSONA = (
    ...
    "You have various skills available as tools:\n"
    "- Avatar actions: dance (dance/Bling-Bang-Bang-Born, shikanoko, spin), acrobatics (backflip, frontflip, flyingkick, punch, jump), poses (peace_sign, bow, kiss, model_pose, shoot), gestures (wave, nod, shake_head, happy, angry), daily (drink_water, sleep, sit, walk, run) (use perform_action)\n"
    "- Date & time: check current time or date (use get_current_time, get_current_date)\n"
    "- Weather: check current weather in Ho Chi Minh City (use get_weather)\n"
    "Use the appropriate tool when the user's request matches a skill capability.\n"
